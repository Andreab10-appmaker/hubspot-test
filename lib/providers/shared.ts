...
// Funzione per inviare un evento SSE al browser.
export type SendFn = (data: object) => void;

// Tool in formato neutro rispetto al provider; ogni provider lo converte nel
// proprio formato (Anthropic: input_schema; OpenAI: function.parameters).
...
// Tool "di visualizzazione": non viene eseguito sul CRM, ma fa renderizzare un
// grafico nell'interfaccia. È identico per entrambi i provider.
...
6. Accompagna SEMPRE i grafici con un breve commento testuale (1-3 frasi): il
   numero chiave, il trend e un suggerimento di azione.
...
export function buildSystemPrompt(): string {
  ...
  return `${SYSTEM_BASE}\n\nData odierna: ${today} (fuso Europe/Rome). Interpreta richieste come "questo mese", "2026" o "prossimi 3 anni" rispetto a questa data.`;
}

// Esegue un tool: render_chart -> evento "chart" + ack; altrimenti -> tool MCP.
